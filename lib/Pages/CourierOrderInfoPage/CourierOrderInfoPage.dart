...
import 'package:fooday_mobile_app/Pages/CourierMapPage/CourierMapPage.dart';
import 'package:fooday_mobile_app/Utils.dart';
import 'package:mysql1/mysql1.dart';

class CourierOrderInfoPage extends StatefulWidget {
  UserOrderData _order;

  CourierOrderInfoPage(this._order);

  @override
  State<StatefulWidget> createState() => _CourierOrderInfoPageState(_order);
}

class _CourierOrderInfoPageState extends State<CourierOrderInfoPage> {
  int _orderId;
  ...
  _CourierOrderInfoPageState(this._order) {
      ...
      Center(
        ...
        onPressed: () async {
          await Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (BuildContext context) => CourierMapPage(_order)));
        },
      )),
      SizedBox(height: 15),
      Center(
        ...
        child: Text("Повідомити про виконання"),
      ))
    ]);
  }

  Future<bool> _onDonePress() async {
    const QUERY = """
    UPDATE delivery_order SET completed = TRUE, completion_datetime = ?, 
      need_payment = FALSE
    WHERE order_id = ?;
    """;
    Results results = await DatabaseConnector.getQueryResultsAsync(
        QUERY, [DateTime.now().toString(), _order.orderId]);
    return results == null;
  }
